...
Quality gates either **pass** or **fail**. There are **NO exceptions, caveats, or conditional passes**. Gates 2-5 are automated script checks. Gate 1 (Functional) and Gate 6 (Spec Compliance) are assessed by humans and LLM agents respectively. All gates follow the same binary pass/fail principle.
...
| Check | Pass | Fail |
|-------|------|------|
| Prettier | Exit code 0 (all files formatted) | 1+ unformatted files |
...
**Type:** LLM-powered (agent-based)
**Agent:** `spec-compliance-watchdog`
**Pass Criteria:**
- All acceptance criteria in the story file are accurately reflected in the implementation
- All test-design scenario Input/Expected tables match what the tests assert and the code produces
- AC-to-Example mapping in the test-handoff document is consistent with the actual tests

**Resolution Options (when gate fails):**

| Option | Action | Who acts |
|--------|--------|----------|
| Option A: Fix code | Developer modifies implementation to match specs | Developer agent |
| Option B: Update specs | Watchdog updates all generated-docs to match code | spec-compliance-watchdog Call B |

**Note:** This gate is executed as an agent call within the QA flow, not by `quality-gates.js`. It runs after manual verification and before the final commit.
...
- Use `.skip()` for any reason — tests must pass or fail, never be skipped
...
6. **Verify FRS compliance** - During code review (Gate 1), check that implementation matches FRS requirements, not just template patterns. Flag cases where template code was extended instead of replaced when the FRS requires a different approach.
