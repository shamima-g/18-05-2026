...
4. **Run quality gates** (all must exit 0):
   - `npm --prefix web test` — all tests pass
   - `npm --prefix web run test:quality` — zero anti-patterns
   - `npm --prefix web run lint` — zero errors/warnings
   - `npm --prefix web run build` — successful build
