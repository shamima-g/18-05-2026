...
3. Identify core business behaviors
   ...
   **Prototype-imported projects:** When `_component-reference.md` exists, apply [`prototype-source-reference.md`](../shared/prototype-source-reference.md) — read the named source file and quote `validate()` rules, declaration text, error messages, and show/hide logic verbatim in your examples.

4. Identify decision gaps or missing rules the BA may not have specified

   **Prototype shortcuts surface BA decisions automatically.** Each `prototypeShortcuts` entry maps to a `(BA-N)` block — "production behaviour is X; prototype shortcut Y can't carry over; please confirm." FRS CR-gaps flagged in the reference doc become BA decisions too (how the content appears, what the test asserts).
