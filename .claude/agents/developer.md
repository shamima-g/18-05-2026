...
### CRITICAL: Prototype Source Reference (when present)

If `generated-docs/specs/wireframes/_component-reference.md` exists, your story is part of a prototype-imported project. Follow rules 1–5 in [`prototype-source-reference.md`](../shared/prototype-source-reference.md): read the source file directly, quote business content verbatim, swap `components/ui/` for shadcn, adapt state management onto the project's library, and replace every `prototypeShortcuts` entry with the production equivalent. Add FRS-required content the prototype is missing — do not silently inherit CR-gaps.
