...
| **INTAKE** | prototype-review-agent | (v2 only) Catalogue `prototype-src/` screens, extract enrichments, flag assumptions, cross-validate specs, pre-map genesis→FRS (once) |
...
**Purpose:** Reviews v2 prototype artifacts — catalogues screens from `documentation/prototype-src/`, extracts domain enrichments, flags prototype-scoped assumptions, cross-validates genesis data structures against OpenAPI specs, and pre-maps genesis sections to FRS sections. `documentation/project.pen` is retained as a designer-intent artifact only (no PNG export, no Pencil MCP).
...
**Key outputs:** Structured return only — `prototypeCatalogue`, `sharedShell`, `enrichments`, `assumptionsFlagged`, `dataStructureMismatches`, section mappings. Consumed by `intake-agent` Call C, `intake-brd-review-agent`, and (at DESIGN) `design-wireframe-agent` Call A — which owns `artifacts.wireframes.generate`. This agent does not write files or mutate the manifest.
...
1. **INTAKE (Mandatory):** Orchestrator welcomes user and routes (share docs, prototype import, or guided Q&A), then runs intake-agent, prototype-review-agent (v2 only), and intake-brd-review-agent via scoped calls
   ...
   - For v2 prototype imports: prototype-review-agent runs between intake-agent and BRD review — catalogues `prototype-src/`, surfaces enrichments, flags assumptions
   - Orchestrator handles all user interaction via AskUserQuestion (with free-text exception for project description)
   - Approve assumptions (Gate 1) → approve FRS (Gate 2) → **mandatory /clear + /continue**
