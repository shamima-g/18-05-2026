...
3. Check for wireframes — note available screens. On prototype-imported projects, prefer `generated-docs/specs/wireframes/_component-reference.md` over per-screen files: the catalogued routes are the screen inventory for decomposition, and per [`prototype-source-reference.md`](../shared/prototype-source-reference.md) each `prototypeShortcuts` entry and each FRS CR-gap typically becomes its own story. Read named source files directly when sizing.
