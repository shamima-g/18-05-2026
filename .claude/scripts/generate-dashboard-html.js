...
// Phase tooltip descriptions — add an entry here whenever a new phase is introduced.
const PHASE_TOOLTIPS = {
  'INTAKE':      'Claude reads your documentation, fills gaps with targeted questions, and produces a Feature Requirements Specification with numbered, testable requirements. This spec drives all subsequent phases.',
  'DESIGN':      'Claude generates only the technical artifacts your feature needs — API spec, CSS design tokens, or wireframes — based on what INTAKE gathered. Artifacts not required for your feature are skipped.',
  'SCOPE':       'Claude divides your requirements into epics (the major work packages) and maps how they depend on each other. No stories or code yet — this phase defines the overall shape and sequence of the work.',
  'STORIES':     'Claude writes the individual stories for the current epic. Each story gets a set of structured acceptance criteria that spell out exactly what the feature must do before any code is written.',
  'REALIGN':     'Before work begins on the next story, Claude checks whether anything discovered during previous stories changes what needs to be built. Surprises are resolved here, not mid-implementation.',
  'TEST-DESIGN': 'Claude drafts a plain-language test plan for the next story. Edge cases and open decisions are captured before any test code is written.',
  'WRITE-TESTS': 'Claude writes tests that are meant to fail at this stage — that is intentional. The next phase writes the code that makes them pass.',
  'IMPLEMENT':   'Claude writes the minimum code needed to make the failing tests pass. No new tests are added here — just the implementation that satisfies what WRITE-TESTS defined.',
  'QA':          'Claude reviews the code and runs five automated quality gates covering security, code quality, tests, performance, and functional behavior. The story is committed only when all gates pass.',
};
...
function pipelineHTML() {
  ...
  const storiesStep = stepHTML('STORIES');
