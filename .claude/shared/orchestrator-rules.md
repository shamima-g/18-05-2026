...
These are two separate actions. A subagent's return is visible to you but **invisible to the user** — if the content lives only in the agent's return, you have NOT displayed it yet.
...
This applies at every approval prompt in every phase (INTAKE manifest, SCOPE epics, STORIES, TEST-DESIGN, QA checklist, spec-compliance reports).

**Fallback if the agent return is empty or unclear:** read the file the agent wrote (e.g., the intake manifest) and construct the summary yourself before calling AUQ.
...
Agents are pre-authorized to run `git push origin main` at designated milestones — the user already approved the underlying work via `AskUserQuestion`. When an agent's instructions say push, execute directly; don't re-confirm. On push failure, report the error and stop — don't retry.

Designated push points: `intake-brd-review-agent` Call B (Gate 2), `feature-planner` SCOPE Call B (epics approved), `feature-planner` STORIES Call B (stories approved), `code-reviewer` Call C (QA pass).
...
|-------|-------|-------|-------------------------------|
| intake-agent | INTAKE | A (scan) + B (manifest) + C (assumptions.md + connectivity merge) + D (revise, conditional) | Onboarding routing + optional project description + 6 checklist Qs (Q1, Q2, Q3a, Q3b, Q4, Q5) delivered in 4 batched AUQ calls + optional Q6 wireframe quality + Gate 1 (assumptions approval) |
| prototype-review-agent | INTAKE (v2 only) | 1 (single call) | None — enrichments and prototype assumptions fold into assumptions.md as `[INFERRED]`/`[DEFAULT]` entries for review at Gate 1 |
| intake-brd-review-agent | INTAKE | A (produce FRS from approved assumptions) + B (finalize/revise, conditional) | Gate 2 (FRS approval) — with bounce-back to Gate 1 when underlying assumption is wrong |
...
| design-roles-agent | DESIGN | A (matrix from template) + B (commit) | 1 approval (matrix sign-off) |
...
- **`agentsToRun`**: agents where `generate == true` — any combination of: `design-api-agent`, `design-style-agent`, `design-wireframe-agent`, `design-roles-agent`. **E6 wireframe skip:** If `artifacts.wireframes.generate == false` (set by design-wireframe-agent Call A in Component Reference Mode), do NOT re-launch `design-wireframe-agent` — `componentReferencePath` (`_component-reference.md`) is the output. **Roles always-on:** `design-roles-agent` always runs in DESIGN; it reads `context.rolesTemplate` from the manifest (always set by INTAKE).
...
See the [Finalize DESIGN](#finalize-design) section below.
